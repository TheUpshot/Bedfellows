...
def initial_setup(cursor):
    ...
    sql.append("UNLOCK TABLES;")
    sql.append("ALTER TABLE fec_contributions ADD INDEX (fec_committee_id, other_id);")
    commit_changes(cursor, sql)
...
def compute_exclusivity_scores(cursor):  
    ...
    sql.append("INSERT INTO total_donated_by_PAC (fec_committee_id, contributor_name, total_by_PAC) SELECT T.fec_committee_id, T.contributor_name, SUM(T.amount) AS total_by_PAC FROM fec_contributions T GROUP BY T.fec_committee_id ORDER BY NULL;")
    sql.append("UNLOCK TABLES;")
    sql.append("ALTER TABLE total_donated_by_PAC ADD INDEX (fec_committee_id);")
    commit_changes(cursor, sql)
    ...
    sql.append("UNLOCK TABLES;")
    sql.append("ALTER TABLE exclusivity_scores ADD INDEX (fec_committee_id, other_id);")
    commit_changes(cursor, sql)
...
def compute_report_type_scores(cursor):
    ...
    sql.append("UNLOCK TABLES;")
    sql.append("ALTER TABLE pairs_count ADD INDEX (fec_committee_id, other_id);")
    commit_changes(cursor, sql)
...
def compute_periodicity_scores(cursor):
    ...
    sql.append("UNLOCK TABLES;")
    sql.append("ALTER TABLE unnormalized_periodicity_scores ADD INDEX (periodicity_score);") # Might need to change this, see overall_score!
    commit_changes(cursor, sql)
    ...
    sql.append("UNLOCK TABLES;")
    sql.append("ALTER TABLE periodicity_scores ADD INDEX (fec_committee_id, other_id);") # Might need to change this, see overall_score!
    commit_changes(cursor, sql)
...
def compute_maxed_out_scores(cursor):
    ...
    for index, row in enumerate(rows):
        if index == 0:
            ...
            sql.append("INSERT INTO contribution_limits (contributor_type, recipient_type, cycle, contribution_limit) VALUES ('%s','%s','%s','%s')" % (row[0], recipient_type_1, row[1], row[2]))
            sql.append("INSERT INTO contribution_limits (contributor_type, recipient_type, cycle, contribution_limit) VALUES ('%s','%s','%s','%s')" % (row[0], recipient_type_2, row[1], row[3]))
            sql.append("INSERT INTO contribution_limits (contributor_type, recipient_type, cycle, contribution_limit) VALUES ('%s','%s','%s','%s')" % (row[0], recipient_type_3, row[1], row[4]))
            sql.append("INSERT INTO contribution_limits (contributor_type, recipient_type, cycle, contribution_limit) VALUES ('%s','%s','%s','%s')" % (row[0], recipient_type_4, row[1], row[5]))
            try:
                cursor.execute(sql1)
                cursor.execute(sql2)
                cursor.execute(sql3)
    ...
    sql.append("UNLOCK TABLES;")
    sql.append("ALTER TABLE unnormalized_maxed_out_scores ADD INDEX (maxed_out_score);")
    commit_changes(cursor, sql)
...
def compute_length_scores(cursor):
    ...
    sql.append("UNLOCK TABLES;")
    sql.append("ALTER TABLE unnormalized_length_scores ADD INDEX (fec_committee_id, other_id, cycle);")
    commit_changes(cursor, sql)
...
def compute_race_focus_scores(cursor):
    ...
    sql.append("UNLOCK TABLES;")
    sql.append("ALTER TABLE races_list ADD INDEX (cycle, district, office_state, branch);")
    commit_changes(cursor, sql)
    ...
    sql.append("UNLOCK TABLES;")
    sql.append("ALTER TABLE race_focus_scores ADD INDEX (fec_committee_id, cycle);")
    commit_changes(cursor, sql)
...
def compute_final_scores(cursor):
    ...
    sql.append("UNLOCK TABLES;")
    sql.append("ALTER TABLE five_scores ADD INDEX (fec_committee_id, other_id, cycle);")
    commit_changes(cursor, sql)
